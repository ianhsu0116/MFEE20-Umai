...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    let member = await new Promise((resolve, reject) => {
      connection.getConnection(("SELECT * FROM member WHERE email = ?", email));
    });
    console.log("Done");
...
router.post("/registration", async (req, res) => {
  ...
  try {
    ...
    // 送新用戶一張優惠券
    let couponsResult = await new Promise((resolve, reject) => {
      connection.getConnection(
...
router.get("/memberInfo/:id", async (req, res) => {
  ...
  try {
    let member = await new Promise((resolve, reject) => {
      connection.getConnection(
        ("SELECT * FROM member WHERE id = ? AND valid = ?", [id, 1])
      );
    });
...
router.post("/findPassword", async (req, res) => {
  ...
  try {
    // 先尋找有無此email
    let foundMember = await new Promise((resolve, reject) => {
      connection.getConnection(
        ("SELECT * FROM member WHERE email = ? AND valid = ?", [email, 1])
      );
    });
