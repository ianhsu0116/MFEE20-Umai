...
function ShoppingList(props) {
  ...
  async function getmembercount() {
    try {
      let result = await axios.get(
        `http://localhost:8080/api/course/${coursetitle.course_id}`,
        {
      ...
      for (let i = 0; i <= result.data.course_batch.length; i++) {
        if (result.data.course_batch[i]["id"] === coursetitle.batch_id)
          setcoursetitle({
            ...coursetitle,
            membercount: result.data.course_batch[i].member_count,
  ...
  async function insertstudentdata() {
    carddata.map(async (data) => {
      ...
      try {
        let inset_student_data = await axios.post(
          API_URL + "/order/insertStudentData",
          studentdata,
          { withCredentials: true }
        );
      } catch (error) {
  ...
  async function modifymembercount() {
    ...
    try {
      let modify_membercount = await axios.put(
        API_URL + "/order/modifyMembercount",
        modifydata,
        { withCredentials: true }
      );
    } catch (error) {
  ...
  async function modifycoupon() {
    try {
      let result = await axios.put(
        `http://localhost:8080/api/order/modifycoupon`,
        {
          id: coupon.id,
          memberid: currentUser.id,
      ...
      <div className="ShoppingBill-background">
        <div className="ShoppingBill-wrapper">
          ...
          <div className="ShoppingBill-payment-details">
            <h3>付款明細</h3>
            <PaymentDetails OrdererData={OrdererData} />
          </div>
        </div>
      </div>
    </>
